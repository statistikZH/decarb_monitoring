...
m4_computed <- m4_cleaned %>%
  ...
  dplyr::left_join(m4_total, by = c("Gebiet", "Jahr")) %>%
...
m4_export_data <- m4_computed %>%
  ...
  dplyr::mutate(Gebiet = dplyr::if_else(Gebiet == "Zürich", "Kanton Zürich", Gebiet),
                Einheit = dplyr::case_when(Einheit == "Wert" ~ "Neuzulassungen Gütertransportfahrzeuge [Anz.]",
                                           Einheit == "Anteil" ~ "Neuzulassungen Gütertransportfahrzeuge [%]",
                                           TRUE ~ Einheit)) %>%
