...
url <- "https://www.uvek-gis.admin.ch/BFE/ogd/92/data-cantons.csv"
m5_bfe <- read.delim(url, header = TRUE, sep = ",")

# Subset mit Jahreszahlen
m5_jahr <- m5_bfe |>
  dplyr::filter((Quarter == 5 | Month == 13) & Year < 2026) |>
  dplyr::select(Kanton, "Jahr" = Year, "Mean_CO2" = t1, "Num_Cars" = t8)
...
m5_export_data <- m5_jahr |>
  dplyr::filter(Kanton == 1) |>
  dplyr::select(Jahr, "Wert" = Mean_CO2) |>
