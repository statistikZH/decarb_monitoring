# EV1 - Produktion von Wärme aus erneuerbaren Energieträgern ----------------------------------------------------


# Import data -------------------------------------------------------------
# Schritt 1 : hier werden die Daten eingelesen

ds <- create_dataset('EV1')
ds <- download_data(ds)
...
EV1_pop <- decarbmonitoring::download_per_capita()%>%
...
EV1_computed <- EV1_data %>%
  dplyr::filter(Energiesektor == "Wärme") %>%
...
EV1_export_data <- EV1_computed %>%
  ...
  dplyr::mutate(Indikator_ID = ds$dataset_id,
                ...
                Variable = "Wärme aus erneuerbaren Energieträgern") %>%
  dplyr::select(Jahr, Gebiet, Indikator_ID, Indikator_Name, Variable, Wert, Einheit, Datenquelle)


# assign data to be exported back to the initial ds object -> ready to export
ds$export_data <- EV1_export_data
